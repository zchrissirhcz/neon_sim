...
{
    //定义a, b, c, d, e, f, g
    uint8_t *a = (uint8_t*)malloc(sizeof(uint8_t) * 256);
    uint8_t *b = (uint8_t*)malloc(sizeof(uint8_t) * 256);
    uint8_t *c = (uint8_t*)malloc(sizeof(uint8_t) * 256);
    uint8_t *d = (uint8_t*)malloc(sizeof(uint8_t) * 256);
    uint8_t *e = (uint8_t*)malloc(sizeof(uint8_t) * 256);
    uint8_t *f = (uint8_t*)malloc(sizeof(uint8_t) * 256);
    uint8_t *g = (uint8_t*)malloc(sizeof(uint8_t) * 256);
    ...
    //-------------------------------------------------------------------------------------------
    //测试vaddl
    uint16_t *d_16 = (uint16_t*)malloc(sizeof(uint32_t) * 256);
    uint16x8_t regd_16;
    for(int i = 0 ; i < 256 ; i += 8)
    {
        rega_8 = vld1_u8(&a[i]);	//加载a
    ...
    //-------------------------------------------------------------------------------------------
    //测试vaddw
    uint16_t *b_16 = (uint16_t*)malloc(sizeof(uint32_t) * 256);
    uint16x8_t regb_16;
    for(int i = 0 ; i < 256 ; i += 8)
    {
        rega_8 = vld1_u8(&a[i]);	//加载a
    ...
    //-------------------------------------------------------------------------------------------
    //测试vmulq_n
    uint32_t *a_32 = (uint32_t*)malloc(sizeof(uint32_t) * 256);
    uint32_t *b_32 = (uint32_t*)malloc(sizeof(uint32_t) * 256);
    for(int i = 0 ; i < 256 ; i++)
    {
        a_32[i] = i;
    }
